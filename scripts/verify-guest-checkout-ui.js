...
async function main() {
  ...
  assert(fs.existsSync('services/frontend/app/api/address-autocomplete/route.ts'), 'Frontend must expose a server-side address autocomplete proxy route for local development');
  assert(fs.existsSync('services/api-gateway/src/gateway/address-autocomplete.service.ts'), 'Gateway must expose Czech RUIAN address autocomplete service');
  assert(read('services/api-gateway/src/gateway/address-autocomplete.service.ts').includes('ruian_address_points') && read('services/api-gateway/src/gateway/gateway.controller.ts').includes("@Get('address-autocomplete')"), 'Address autocomplete must use the local RUIAN database through api-gateway');
  assert(fs.existsSync('prisma/migrations/20260701_ruian_address_points/migration.sql'), 'RUIAN address table migration must exist');
  assert(read('scripts/import-ruian-addresses.js').includes('services.cuzk.gov.cz') && read('scripts/import-ruian-addresses.js').includes('VFR') === false, 'RUIAN import script must use the free CUZK services source without Google Places');
