...
Our application flipflop.alfares.cz consists of several microservices in ~/Documents/Github/ (payment, logging, nginx, database, notifications, flipflop). Access via ssh alfares.

ssh alfares "cd flipflop && git pull"

pull github repos using ssh alfares "cd flipflop && git pull && cd ../nginx-microservice && git pull && docker exec nginx-microservice nginx -t && docker exec nginx-microservice nginx -s reload"
...
Run: ssh alfares "cd nginx-microservice && ./scripts/blue-green/deploy-smart.sh flipflop"
