2026-07-03: Owner approved Goal 24 discount/price fixture path to keep the exact linked Fiobanka paid/provider smoke at checkout-authoritative total `<= 300 CZK`. Source inspection rejected direct client `discount` as unsafe; accepted path is a guarded server-validated fixed discount code. Current target total remains `1998 CZK`; deterministic fixture is fixed `1698 CZK` discount, one use, short expiry, Goal 24 correlated, producing final amount `300 CZK`. This does not authorize Catalog price mutation, marketplace/feed/listing mutation, persistent product price changes, direct DB row edits, direct client discount override, secret/token output, raw customer/order/payment/provider data, deploy, or migration. Runtime side effects remain gated to one exact server-validated discount-code attempt and stop at first hard stop.
