2026-07-06: W5 deployed assertion source config wired source-only. [RESOLVED/NARROWED: k8s/configmap.yaml now carries non-secret JSONL assertion source env vars for synthetic email and event trace capture] Deploy/readback remains required before another W5 runtime attempt; no deploy, checkout, order, payment, provider, email, event, CRM/Leads readback, DB write, or secret output occurred. Remaining blockers: [MISSING: deployed ConfigMap/runtime env readback for synthetic email/event assertion sources], [MISSING: CRM/Leads no-op or retention acknowledgement], [MISSING: sandbox/test-mode payment success evidence for invoice pending/no-provider].
2026-07-06: W5 deployed assertion env source prepared. [RESOLVED/NARROWED: k8s/configmap.yaml now carries SYNTHETIC_EMAIL_ASSERTION_SOURCE=synthetic-email-jsonl:reports/validation/synthetic-email-assertions/email-assertions.jsonl; SYNTHETIC_EMAIL_ASSERTION_DOMAIN=example.invalid; SYNTHETIC_EVENT_TRACE_SOURCE=synthetic-event-trace-jsonl:reports/validation/customer-journey-event-trace/events.jsonl] Runtime readback remains blocked by [MISSING: live ConfigMap apply/restart/readback evidence], [MISSING: CRM no-op/retention acknowledgement], and [MISSING: sandbox/test-mode payment success evidence; invoice remains pending/no-provider]. Boundary: checkout_submission=false; order_created=false; payment_created=false; provider_call=false; webhook_replay=false; email_sent=false; event_published_by_packet=false; orders_mutation=false; warehouse_mutation=false; deploy=false; db_write=false; secret_output=false; raw_customer_or_payment_evidence=false. Verifier: npm run verify:customer-journey-deployed-assertion-env-source.
