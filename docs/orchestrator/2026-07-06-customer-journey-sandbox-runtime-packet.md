...
| Event trace assertion source | `[RESOLVED/NARROWED: approved source contract is SYNTHETIC_EVENT_TRACE_SOURCE=synthetic-event-trace-jsonl:reports/validation/customer-journey-event-trace/events.jsonl; customer journey publisher records sanitized JSONL trace rows before broker connectivity, queryable by journey_id/correlation_id even when broker publish is unavailable]` | query path for `flipflop.customer_journey.events.v1` by `journey_id`/`correlation_id` |
...
Decision: `[RESOLVED/NARROWED: W5 central Orders paid lifecycle evidence exists for the owner-approved synthetic no-provider handoff]`.

Runtime evidence:

```text
ordersPaymentStatusContract=orders.payment-status.v1
...
```text
reports/validation/VAL-W5-central-orders-paid-lifecycle-evidence-2026-07-07.md
